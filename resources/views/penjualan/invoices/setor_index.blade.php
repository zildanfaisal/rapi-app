...
<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
    ...
    <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-6">
        ...
        <div class="text-4xl font-extrabold text-yellow-800">
            @php $amt = (int) ($notDepositedTotal ?? 0); @endphp
            {{ $amt > 0 ? '- Rp '.number_format($amt,0,',','.') : 'Rp 0' }}
        </div>
    ...
    <div class="bg-white border border-gray-200 rounded-lg p-6">
        <form method="GET" action="{{ route('invoices.setor') }}" class="grid gap-4">
            ...
            <div class="flex justify-end gap-2">
                <button class="px-4 py-2.5 bg-blue-600 text-white rounded-lg">Filter</button>
                <a href="{{ route('invoices.setor') }}"
                   class="px-4 py-2.5 bg-gray-200 rounded-lg">Reset</a>
            </div>
        </form>
    </div>
</div>
...
<tr class="text-center hover:bg-gray-50">
    ...
    <td class="border px-3 py-2">Rp {{ number_format($inv->grand_total,0,',','.') }}</td>
...
function handleResponsive(){
    if(window.innerWidth>=1024){
        ...
        if(!dataTable){
            dataTable=new DataTable('#dataTablesSetor',{responsive:true});
        }
    }else{
